...
def manual_fix():
    ...
    # Show the mask we want and turn on 3D priview mode
    for m in mimics.data.masks:
      m.selected = False
...
if __name__ == '__main__':
  ...
  # Get a list of all the orbit.mcs files in root
  # These have been pre-processed to mostly segment the Orbital volumes, but need manual tweaking doen here.
  projects = [f.path for f in os.scandir(root) if re.match(r'.*orbit.mcs', f.name)]
  
  # Only look at a subset to test
  #projects = projects[0:1]

  num_projects = len(projects)
  print(f'02_manual_cleanup - processing {num_projects} project files from {root}')

  for i, p in enumerate(projects):
      mimics.file.open_project(filename=p, read_only_mode=False)
      ...
      path_name = os.path.join(output_folder, p) # full path for output file
      mimics.file.save_project()

      mimics.file.close_project()

  print(f'02_manual_cleanup - finished {num_projects} project files ------------------------------')
