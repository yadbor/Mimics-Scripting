...
def make_clean():
    ...
    soft_orbits = mimics.data.masks["soft_orbits"]
    try:
        soft_clean = mimics.data.masks["soft_clean"]
    except KeyError:
        soft_clean = mimics.data.masks.duplicate(object=soft_orbits)
        soft_clean.name = "soft_clean"
...
def make_separate():
    ## Create the globe masks and add to the soft_clean mask
    try:
        globe_left = mimics.data.masks["globe_left"]
    except KeyError:
        globe_left = utils.sphere_to_mask(eyes_left)
        globe_left.name ="left_globe"
    globe_left.visible = False
    ...
    try:
        globe_right = mimics.data.masks["globe_right"]
    except KeyError:
        globe_right = utils.sphere_to_mask(eyes_right)
        globe_right.name ="right_globe"
    globe_right.visible = False
...
if __name__ == '__main__':
  ...
  answer = mimics.dialogs.question_box(message="Check the masks and clean up as needed, then continue.", buttons="Make_clean;Separate;Done", ui_blocking=False)
  while answer != "Done":
    if answer == "Make_clean":
        make_clean()
    if answer == "Separate":
      try:
        soft_clean = mimics.data.masks["soft_clean"]
