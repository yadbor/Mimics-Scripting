...
def make_masks(bb):
    ...
    bone_temp = mimics.segment.calculate_part(mask=bone_orbit, quality='Optimal') # Failed on 'High', try Optimal
    ...
    # This failed, saying the part was not a solid. Catch any errors and skip this bit if needed
    try:
        bone_wrap = mimics.segment.calculate_mask_from_part(part=bone_part, target_mask=None)
        bone_wrap.name = "bone_wrap"
    except ValueError:
        # Fails if the part is not a solid, so return that somehow
        bone_wrap = None
    ...
    for p1, p2 in utils.looped_pairwise(rim.geometry_points):
      ...
      if (p1[Z] >= plane_z and p2[Z] < plane_z): # crosses from above
        ...
        try:
          ...
          # Fails if line too close to parallel to plane, so say we didn't get an intersection
          pt_up = None
...
def pre_manual():
    ...
    p2_offset = (10, 10, 25)    # added to the opposite point of the bounding box. Made Z taller as some cases were clipping top of orbit.
    ...
    with mimics.disabled_gui_update():
      ...
      if bone_wrap is None:
        print("failed to wrap bone - fall back to using smoothed mask")
        bone_wrap = bone_orbit
      ...
      anterior_right = make_anterior_mask(mimics.data.splines["right_rim"], mimics.data.spheres["right_globe"])
      anterior_right.name = "anterior_right"
      anterior_right.visible = False
      anterior_left = make_anterior_mask(mimics.data.splines["left_rim"], mimics.data.spheres["left_globe"])
    ...
    ## Manual part removed for this code - done later as a seperate process to speed things up.
...
if __name__ == '__main__':
  ...
  print(f'01_pre_manual - processing {num_projects} project files from {root}')

  #with mimics.disabled_gui_update():
  for i, p in enumerate(projects):

      # Crashed analysing file 29 "D:\Projects & Research\Enophthalmos Study\Sahtoot 2022.05.26 bone SS 01.mcs"
      # Added error catching for lines parallel to plane
      # Restart from 29 (skip everything up to there)
      if i < 28:
        continue
      ...
      pre_manual() # do the processing up to the manual cleaning up of the Orbital Volume masks
  ...
  print(f'01_pre_manual - finished {num_projects} project files ------------------------------')
